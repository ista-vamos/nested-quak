...
static bool eval_cached_flat(NestedAutomaton* nwa,
                             value_function_t infVal,
                             value_function_t finVal,
                             weight_t threshold) {
    Automaton* flat = NestedAutomatonTester::flatten_MinMax_Inf_cached(nwa, finVal, threshold);
...
static void run_case(const std::string& path,
    ...
    const bool cached = eval_cached_flat(&nwa, infVal, finVal, threshold);
